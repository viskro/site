...
<div x-data="addToCartForm()" class="space-y-6">
    ...
    <div class="flex items-center space-x-4">
        <label class="text-gray-300 font-medium">{{ __('Quantité') }}</label>
    ...
    <div class="space-y-3">
        @if($product->canBePurchased())
            ...
            <button
                ...
                :class="{ 'bg-green-500 text-white hover:bg-green-600': justAdded }"
            >
                <!-- Loading spinner -->
                ...
                <span x-text="buttonText">{{ __('Ajouter au panier') }}</span>
            </button>
        ...
        @else
            ...
            <div class="w-full p-4 bg-red-900/20 border border-red-700 rounded-lg text-center">
                <div class="flex items-center justify-center space-x-2 text-red-400">
                    ...
                    <span class="font-medium">{{ __('Produit temporairement indisponible') }}</span>
                </div>
                <p class="text-red-300 text-sm mt-2">{{ __('Nous travaillons à remettre ce produit en stock très bientôt.') }}</p>
            </div>
            ...
            <button class="w-full flex items-center justify-center px-8 py-4 border-2 border-gray-600 text-gray-300 font-semibold rounded-full hover:border-gray-500 hover:text-white transition-all duration-200">
                ...
                {{ __('Être notifié du retour en stock') }}
            </button>
        @endif
    </div>
...
<script>
    function addToCartForm() {
        return {
            ...
            get buttonText() {
                if (this.justAdded) return '{{ __("Ajouté !") }}';
                if (this.isLoading) return '{{ __("Ajout en cours...") }}';
                return '{{ __("Ajouter au panier") }}';
            },
            ...
            async addToCart() {
                ...
                try {
                    ...
                    this.message = 'Une erreur est survenue lors de l\'ajout au panier.';
                    this.messageType = 'error';

<x-app-layout>
    <x-slot name="title">{{ __('Paiement') }}</x-slot>
                ...
                <div class="mb-8">
                    <div class="flex items-center justify-center space-x-4 mb-6">
                        <div class="flex items-center text-green-400">
                            <div class="w-8 h-8 bg-green-400 text-gray-900 rounded-full flex items-center justify-center text-sm font-bold">✓</div>
                    ...
                    <h1 class="font-display font-bold text-4xl text-white text-center">{{ __('Paiement') }}</h1>
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <!-- Formulaire de paiement -->
                    <div class="bg-gray-800 rounded-2xl border border-gray-700 p-8">
                        <h2 class="font-display font-bold text-2xl text-white mb-6">{{ __('Informations de paiement') }}</h2>
                        ...
                        <form id="payment-form" class="space-y-6">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">{{ __('Email de confirmation') }}</label>
                            ...
                            <!-- Carte de crédit -->
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">{{ __('Informations de la carte') }}</label>
                            ...
                            <div class="flex items-center space-x-3 p-4 bg-gray-700/50 rounded-lg">
                                ...
                                <div>
                                    <p class="text-white font-medium text-sm">{{ __('Paiement sécurisé') }}</p>
                                    <p class="text-gray-400 text-xs">{{ __('Vos informations sont protégées par Stripe') }}</p>
                                </div>
                            </div>

                            <!-- Bouton de paiement -->
                            <button type="submit" id="submit-payment"
                                    class="w-full bg-white text-gray-900 font-bold py-4 px-6 rounded-full hover:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                                <span id="button-text">{{ __('Payer') }} {{ number_format($summary['total'], 2) }} €</span>
                            ...
                            <div class="text-center">
                                ...
                                <p class="text-gray-400 text-xs">{{ __('Paiement sécurisé SSL 256-bit') }}</p>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="bg-gray-800 rounded-2xl border border-gray-700 p-8">
                        <h2 class="font-display font-bold text-2xl text-white mb-6">{{ __('Récapitulatif') }}</h2>
                            ...
                            @foreach($cartItems as $item)
                                <div class="flex items-center space-x-4 p-4 bg-gray-700 rounded-lg">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-gray-400 text-sm">{{ __('Quantité: :qty', ['qty' => $item['quantity']]) }}</p>
                                    </div>
                        ...
                        <div class="space-y-3 border-t border-gray-600 pt-6">
                            ...
                            <div class="flex justify-between text-gray-400">
                                ...
                                <span>{{ $summary['shipping_cost'] > 0 ? number_format($summary['shipping_cost'], 2) . ' €' : 'Gratuite' }}</span>
                            </div>
                            <div class="flex justify-between text-gray-400 text-sm">
                                <span>{{ __('TVA incluse') }}</span>
                        ...
                        <div class="mt-6 pt-6 border-t border-gray-600 space-y-4">
                            <div>
                                <h4 class="text-white font-medium mb-2">{{ __('Adresse de livraison') }}</h4>

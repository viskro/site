<x-app-layout>
    <x-slot name="title">{{ __('Mentions légales') }}</x-slot>

    <div class="min-h-screen bg-gray-900 py-12">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="max-w-4xl mx-auto bg-gray-800 rounded-2xl border border-gray-700 p-8">
                <x-layout.breadcrumb :items="[['label' => __('Accueil'), 'url' => route('home')], ['label' => __('Mentions légales')]]" />
                <h1 class="font-display font-bold text-4xl text-white mb-2">{{ __('Mentions légales') }}</h1>
                <p class="text-gray-400 mb-8">{{ __('Informations obligatoires concernant l’éditeur et l’hébergement du site.') }}</p>

                <div class="space-y-6">
                    <section>
                        <h2 class="text-white text-2xl font-semibold mb-2">{{ __('Éditeur du site') }}</h2>
                    ...
                    <section>
                        <h2 class="text-white text-2xl font-semibold mb-2">{{ __('Contact') }}</h2>
                        <p class="text-gray-300 leading-relaxed">Email: support@soundtags.fr<br>Téléphone: +33 1 23 45 67 89</p>
                    </section>

                    <section>
                        <h2 class="text-white text-2xl font-semibold mb-2">{{ __('Directeur de la publication') }}</h2>
                        <p class="text-gray-300 leading-relaxed">Nom du directeur de publication</p>
                    </section>

                    <section>
                        <h2 class="text-white text-2xl font-semibold mb-2">{{ __('Hébergement') }}</h2>
                        <p class="text-gray-300 leading-relaxed">Nom de l’hébergeur<br>Adresse de l’hébergeur</p>
                    </section>

                    <section>
                        <h2 class="text-white text-2xl font-semibold mb-2">{{ __('Propriété intellectuelle') }}</h2>
                    ...
                    <section>
                        <h2 class="text-white text-2xl font-semibold mb-2">{{ __('Responsabilité') }}</h2>
